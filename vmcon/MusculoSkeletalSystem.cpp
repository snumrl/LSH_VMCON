...
MusculoSkeletalSystem()
	:mTendonStiffness(1E5),mMuscleStiffness(5E5),mYoungsModulus(3E6),mPoissonRatio(0.3)
{
...
void MakeSkeleton(std::shared_ptr<MusculoSkeletalSystem>& ms)
{
	...
	MakeBody(skel,skel->getBodyNode("Torso"),"NeckR",
		...
		Eigen::Vector3d(0.1,0.0,0.0),JOINT_TYPE::REVOLUTE,3.0);
	...
	MakeBody(skel,skel->getBodyNode("NeckR"),"ShoulderR",
		...
		Eigen::Vector3d(0.15,0.0,0),JOINT_TYPE::BALL_AND_SOCKET,2.0);
	...
	MakeBody(skel,skel->getBodyNode("ShoulderR"),"ElbowR",
		...
		Eigen::Vector3d(0.15,0.0,0),JOINT_TYPE::REVOLUTE,1.0);
	...
	skel->getDof(0)->setPositionLimits(-0.2,0.3);
	skel->getDof(1)->setPositionLimits(0.0,1.57); //X
	skel->getDof(2)->setPositionLimits(-0.2,2.0); //Y
	skel->getDof(3)->setPositionLimits(-0.75,1.8); //Z
	skel->getDof(4)->setPositionLimits(0.1,2.2); 
